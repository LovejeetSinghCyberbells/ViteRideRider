...
import { loginUser, clearError } from '../../redux/fetures/authSlice';
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity
        onPress={() => navigation.goBack()}
        activeOpacity={0.8}
        disabled={isAuthLoading} // Prevent back navigation while logging in
      >
        <MaterialDesignIcons
          name="chevron-left"
          size={40}
      ...
      <View style={styles.card}>
        ...
        <CommonTextField
          ...
          onChangeText={setEmail}
          keyboardType="email-address"
          editable={!isAuthLoading} // Disable input while loading
        />
        <View style={{ height: 20 }} />
        <CommonTextField
        ...
        <Text style={styles.labelText}>
          Don’t have an account?{' '}
          <Text
            style={[styles.labelHighlight, isAuthLoading && { opacity: 0.5 }]}
            onPress={() => !isAuthLoading && navigation.replace('SignUpScreen')}
          >
            Sign up
          </Text>
        </Text>
